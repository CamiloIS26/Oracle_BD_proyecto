...
def handle_actualizar_empleado():
    ...
    while True:
        ...
        if op == "1":
            ...
            if not nuevo.replace('.', '', 1).isdigit(): # Allows for float
        ...
        if mensaje: # Only send if a valid option was selected
...
while True:
    ...
    if opcion == "1":
        ...
        send_and_receive(cliente, "SALIR") # Ensure 'SALIR' command is sent before closing
        break
    else:
        print("Opción inválida. Intente de nuevo.")
    
    input("\nPresione Enter para continuar...") # Pausa para que el usuario vea la salida
